...
static void example_cb(lv_event_t * e)
{
    lv_obj_clean(lv_screen_active());
    lv_example_anim_2();
}
...
void mcp_app(void) {
    ...
    btn = lv_list_add_button(list, NULL, "gb");
    lv_group_remove_obj(btn);
    lv_obj_add_event_cb(btn, choice_cb, LV_EVENT_CLICKED, gb_app);
#endif
#ifdef CONFIG_CUSTOM_APPS_LVGL_APP_XKCD_APP
    btn = lv_list_add_button(list, NULL, "xkcd");
    lv_group_remove_obj(btn);
    lv_obj_add_event_cb(btn, choice_cb, LV_EVENT_CLICKED, xkcd_app);
#endif
    btn = lv_list_add_button(list, NULL, "example");
    lv_group_remove_obj(btn);
    lv_obj_add_event_cb(btn, example_cb, LV_EVENT_CLICKED, NULL);
